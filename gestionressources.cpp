...
bool GestionRessources::ajouterRessource(QString nom2, int quantite, QString categorie, int fournisseur, QDate date_achat, QString etat, double prix, QDate date_entretien)
{
    QSqlQuery query;
...
bool GestionRessources::decrementScannerQuantity() {
    QSqlQuery query;
    query.prepare("UPDATE RESSOURCES SET QUANTITE = QUANTITE - 1 "
                  "WHERE ID_RESSOURCE = 9 AND QUANTITE > 0");

    if (!query.exec()) {
        qDebug() << "Decrement error:" << query.lastError().text();
        return false;
    }

    int rowsAffected = query.numRowsAffected();
    qDebug() << "Decrement successful. Rows affected:" << rowsAffected;

    // Ensure the database transaction is committed
    QSqlDatabase::database().commit();

    return rowsAffected > 0;
}

int GestionRessources::getScannerQuantity() {
    QSqlQuery query;
    query.prepare("SELECT QUANTITE FROM RESSOURCES WHERE ID_RESSOURCE = 9");

    if (!query.exec()) {
        qDebug() << "Get quantity error:" << query.lastError().text();
        return -1;
    }
...
QString GestionRessources::getScannerName() {
    QSqlQuery query;
    query.prepare("SELECT NOM_RESSOURCE FROM RESSOURCES WHERE ID_RESSOURCE = 9");
    if (query.exec() && query.next()) {
        return query.value(0).toString();
    }
    return "Scanner"; // Fallback name
}

bool GestionRessources::updateScannerQuantity(int newQuantity) {
    QSqlQuery query;
    query.prepare("UPDATE RESSOURCES SET QUANTITE = :qty WHERE ID_RESSOURCE = 9");
